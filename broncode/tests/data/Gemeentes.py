...
def __GIO (subdir, jaar, multiVlakken, attribuut):
    gioPad = os.path.join (testscenario_dir, subdir, 'gemeenten_' + str(jaar) + '.gml')
...
symbolisatie.MaakReadme ([testscenario_dir, '01 demo - gemeentegrenzen'], '''#GIO met de gemeentegrenzen
...
''')
symbolisatie.MaakReadme ([testscenario_dir, '02 vlakken - geometrie - multi-geometrie'], '''#GIO met alleen geometrie
...
symbolisatie.MaakReadme ([testscenario_dir, '03 vlakken - GIO-delen'], '''#GIO met GIO-delen
...
Ook als de geometrie van een gebied niet wijzigt in een volgende versie, dan heeft het gebied toch een andere basisgeometrie-ID.
''')
symbolisatie.MaakReadme ([testscenario_dir, '03 vlakken - GIO-delen - multi-geometrie'], '''#GIO met GIO-delen
...
symbolisatie.MaakReadme ([testscenario_dir, '04 vlakken - normwaarden'], '''#GIO met normwaarden
...
Ook als de geometrie van een gebied niet wijzigt in een volgende versie, dan heeft het gebied toch een andere basisgeometrie-ID.
''')
symbolisatie.MaakReadme ([testscenario_dir, '04 vlakken - normwaarden - multi-geometrie'], '''#GIO met normwaarden
...
for mapPad in symbolisatie.GIOMappen ():
    symbolisatie.MaakSpecificatie (mapPad, ['gio_wijziging.json'], {
        'geometrie': [{ "pad": 'gemeenten_' + str(jaar) + '.gml' } for jaar in jaren],
        "wijziging": [
            *[{ "was": 'gemeenten_' + str(jaar-1) + '.gml', "wordt": 'gemeenten_' + str(jaar) + '.gml' } for jaar in jaren if jaar != jaren[0]],
            { "was": 'gemeenten_' + str(jaren[0]) + '.gml', "wordt": 'gemeenten_' + str(jaren[-1]) + '.gml' }
        ],
